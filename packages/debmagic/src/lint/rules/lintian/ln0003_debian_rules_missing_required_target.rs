...
use crate::lint::file_tree::{EntryKind, FileTree};
...
declare_rule! {
    ...
    /// suppresses the check. Inspects `debian/rules` on the packaging tree.
...
impl SourceRule for DebianRulesMissingRequiredTarget {
    fn run(&self, ctx: &mut SourceContext<'_>) {
        let Some(contents) = ctx.files().and_then(read_rules_from_tree) else {
            return;
        };

        for target in missing_policy_targets(&contents) {
...
/// Policy 4.9 required `debian/rules` targets. The diagnostic extra is one of
/// these names.
const POLICY_TARGETS: &[&str] = &[
    "build",
    "build-arch",
    "build-indep",
    "binary",
...
/// `debian/rules` contents, following a symlink. Missing files, unreadable
/// files, and non-UTF-8 contents are unavailable (lintian returns before
/// scanning). An empty file is available and reports no missing targets.
#[cfg(test)]
...
fn read_rules_from_tree(files: &FileTree) -> Option<String> {
    let entry = files.get("debian/rules")?;
    match entry.kind() {
        EntryKind::RegularFile | EntryKind::Hardlink | EntryKind::Symlink => {}
        EntryKind::Directory => return None,
    }
    let bytes = files.read(entry).ok()?;
    String::from_utf8(bytes).ok()
}

/// Unknown makefile includes suppress missing-target reporting (lintian will
/// not chase those files).
...
mod tests {
    ...
    fn missing_file_is_unavailable() {
        ...
        assert!(read_rules(&path).is_none());
    }

    #[test]
    fn non_utf8_file_is_unavailable() {
        let dir = TempDir::new().expect("tempdir");
        let path = dir.path().join("rules");
        fs::write(&path, [0xff, 0xfe]).expect("write rules");
        assert!(read_rules(&path).is_none());
    }

    #[test]
    fn empty_file_reports_no_missing_targets() {
        assert!(missing_policy_targets("").is_empty());
    }

    #[test]
    fn shebang_only_misses_every_policy_target() {
        assert_eq!(
            missing_policy_targets("#!/usr/bin/make -f\n"),
            POLICY_TARGETS
                .iter()
                .map(|target| (*target).to_string())
                .collect::<Vec<_>>()
    ...
    fn percent_target_provides_every_policy_target() {
        assert!(missing_policy_targets("#!/usr/bin/make -f\n\n%:\n\tdh $@\n").is_empty());
    }
    ...
    #[test]
    fn read_rules_reads_a_regular_file() {
        let dir = TempDir::new().expect("tempdir");
        let path = dir.path().join("rules");
        fs::write(&path, "#!/usr/bin/make -f\n\n%:\n\tdh $@\n").expect("write rules");
        let contents = read_rules(&path).expect("read");
        assert!(missing_policy_targets(&contents).is_empty());
    }
}
